...
use FireHub\Core\Support\Enums\ {
    String\Count\CharacterMode, String\Count\WordFormat, Side
};
...
final class StrSB extends StrSafe {
     ...
     * @since 1.0.0
     *
     * @uses \FireHub\Core\Support\LowLevel\StrSafe::addSlashes() To call a parent method if the $characters parameter
     * wasn't provided.
     *
    ...
    public static function pad (string $string, int $length, string $pad = " ", Side $side = Side::RIGHT):string {
        ...
        return $pad !== '' ? str_pad($string, $length, $pad, match ($side) {
            ...
            Side::RIGHT => 1,
            default => 2
        }) : throw new EmptyPadException()->fromString($string);
     ...
     * @param-out int $count
     *
     * @return ($string is empty ? '' : non-empty-string) String with the replaced values.
     *
     ...
     * </p>
     *
     * @return ($string is empty ? '' : non-falsy-string) String with the replaced values.
     */
     ...
     * </p>
     *
     * @return ($string is empty ? '' : non-empty-string) The given string wrapped at the specified column.
     */
     ...
     * </p>
     *
     * @return ($string is empty ? '' : non-empty-string) The trimmed string.
     *
     ...
     * </p>
     *
     * @return ($string is empty ? '' : non-falsy-string) The portion of string specified by the start and length
     * parameters.
     */
     ...
     * @since 1.0.0
     *
     * @uses \FireHub\Core\Support\Enums\String\Count\CharacterMode::ARR_POSITIVE As default parameter.
     *
     * @param string $string <p>
     * The examined string.
     * </p>
     * @param \FireHub\Core\Support\Enums\String\Count\CharacterMode $mode [optional] <p>
     * Count mode.
     * </p>
     *
     ...
     * @throws \FireHub\Core\Support\Exceptions\Str\ChunkLengthLessThanOneException If the length is less than 1.
     *
     * @return list<non-empty-string> If the optional $length parameter is specified, the returned array will be broken down
     ...
     * @since 1.0.0
     *
     * @uses \FireHub\Core\Support\Enums\String\Count\WordFormat::WORDS As default parameter.
     *
     * @param string $string <p>
     * The string.
     * </p>
     ...
     * </p>
     * @param \FireHub\Core\Support\Enums\String\Count\WordFormat $format [optional] <p>
    ...
    public static function lastPosition (string $search, string $string, bool $case_sensitive = true, int $offset = 0):int|false {
        ...
        if ($case_sensitive) return strrpos($string, $search, $offset); // @phpstan-ignore return.type

        return strripos($string, $search, $offset); // @phpstan-ignore return.type
